<?php
    ...
    if($preresult && $preresult->num_rows > 0){
        ...
        if($preresult2 && $preresult2->num_rows > 0) $dbquery = "UPDATE featured_items SET landing_page_visibility = 1 WHERE item_id = $bookID;";
        else $dbquery = "INSERT INTO featured_items (item_id, landing_page_visibility, slider_visibility) VALUES ($bookID, 1, 0)";
        $result = $conn->query($dbquery);
        if($result){
            $return_array["return_title"] = "Success";
            $return_array["return_message"] = "The item added to featured posts successfully. ";
