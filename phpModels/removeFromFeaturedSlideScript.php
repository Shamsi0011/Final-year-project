<?php
    ...
    if($preresult && $preresult->num_rows > 0){
        $dbquery = "UPDATE featured_items SET slider_visibility = 0 WHERE item_id = $bookID;";
        $result = $conn->query($dbquery);
        if($result){
            $return_array["return_title"] = "Success";
            $return_array["return_message"] = "The item removed from featured slider successfully. ";
